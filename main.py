...
async def admin_delete_data(request: AdminDeleteRequest, username: Optional[str] = None):
    try:
        if request.table not in ["processed_items", "label_process_history", "skus"]:
        ...
        chunk_size = 50
        total_deleted = 0
        for i in range(0, len(request.ids), chunk_size):
            chunk = request.ids[i:i + chunk_size]
            id_filter = ','.join([f'"{str(pid)}"' for pid in chunk])
            await supabase_fetch("DELETE", f"{request.table}?{id_column}=in.({urllib.parse.quote(id_filter)})")
...
async def add_sku_vip_10k(request: Request):
    try:
        ...
        data = [{"sku": sku}]
        await supabase_fetch("POST", "sku_vip_10k", data=data, headers={"Prefer": "resolution=ignore-duplicates"})
        return {"success": True}
    except Exception as e:
        print(f"Error adding SKU VIP 10K: {e}")
        raise HTTPException(status_code=500, detail=str(e))

@app.delete("/settings/sku-vip-10k/{sku}")
async def delete_sku_vip_10k(sku: str):
    try:
        await supabase_fetch("DELETE", f"sku_vip_10k?sku=eq.{sku}")
        return {"success": True}
    except Exception as e:
        print(f"Error deleting SKU VIP 10K: {e}")
        raise HTTPException(status_code=500, detail=str(e))

@app.post("/settings/sku-vip-10k/bulk-delete")
async def bulk_delete_sku_vip_10k(request: Request):
    try:
        body = await request.json()
        ids = body.get("ids", [])
        if not ids:
...
@app.post("/settings/sku-vip-10k/export")
async def export_sku_vip_10k(request: Request):
    try:
        body = await request.json()
        ids = body.get("ids", [])
        ...
        output = io.BytesIO()
        with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
            df.to_excel(writer, index=False, sheet_name='SKU_VIP_10K')
        
        output.seek(0)
        return StreamingResponse(
            output,
            media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            headers={"Content-Disposition": "attachment; filename=Export_SKU_VIP_10K.xlsx"}
        )
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@app.post("/settings/import-sku-vip-10k")
async def import_sku_vip_10k(file: UploadFile = File(...)):
    try:
        content = await file.read()
        df = pd.read_excel(io.BytesIO(content), dtype=str)
...
@app.post("/toolkit/orderan-kilat-10k")
async def process_orderan_kilat_10k(file: UploadFile = File(...)):
    try:
        content = await file.read()
        df = pd.read_excel(io.BytesIO(content), dtype=str, keep_default_na=False)
        ...
        return StreamingResponse(
            output,
            media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            headers={"Content-Disposition": "attachment; filename=Hasil_Orderan_Kilat_10K.xlsx"}
        )
...
async def add_sku_vip_20k(request: Request):
    try:
        ...
        data = [{"sku": sku}]
        await supabase_fetch("POST", "sku_vip_20k", data=data, headers={"Prefer": "resolution=ignore-duplicates"})
        return {"success": True}
    except Exception as e:
        print(f"Error adding SKU VIP 20K: {e}")
        raise HTTPException(status_code=500, detail=str(e))

@app.delete("/settings/sku-vip-20k/{sku}")
async def delete_sku_vip_20k(sku: str):
    try:
        await supabase_fetch("DELETE", f"sku_vip_20k?sku=eq.{sku}")
        return {"success": True}
    except Exception as e:
        print(f"Error deleting SKU VIP 20K: {e}")
        raise HTTPException(status_code=500, detail=str(e))

@app.post("/settings/sku-vip-20k/bulk-delete")
async def bulk_delete_sku_vip_20k(request: Request):
    try:
        body = await request.json()
        ids = body.get("ids", [])
        if not ids:
...
@app.post("/settings/sku-vip-20k/export")
async def export_sku_vip_20k(request: Request):
    try:
        body = await request.json()
        ids = body.get("ids", [])
        ...
        output = io.BytesIO()
        with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
            df.to_excel(writer, index=False, sheet_name='SKU_VIP_20K')
        
        output.seek(0)
        return StreamingResponse(
            output,
            media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            headers={"Content-Disposition": "attachment; filename=Export_SKU_VIP_20K.xlsx"}
        )
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@app.post("/settings/import-sku-vip-20k")
async def import_sku_vip_20k(file: UploadFile = File(...)):
    try:
        content = await file.read()
        df = pd.read_excel(io.BytesIO(content), dtype=str)
...
@app.post("/toolkit/orderan-kilat-20k")
async def process_orderan_kilat_20k(file: UploadFile = File(...)):
    try:
        content = await file.read()
        df = pd.read_excel(io.BytesIO(content), dtype=str, keep_default_na=False)
        ...
        return StreamingResponse(
            output,
            media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            headers={"Content-Disposition": "attachment; filename=Hasil_Orderan_Kilat_20K.xlsx"}
        )
