...
interface AdminSkuPriorityProps {
    showToast?: (message: string) => void;
}

const AdminSkuPriority: React.FC<AdminSkuPriorityProps> = ({ showToast }) => {
    ...
    const fetchPriorityList = async () => {
        setIsLoading(true);
        try {
            const res = await axios.get(`${API_CONFIG.BASE_URL}/settings/priority-bottom`);
            const sorted = [...(res.data || [])].sort((a: PriorityItem, b: PriorityItem) => 
    ...
    const handleSaveSelection = async () => {
        ...
        try {
            ...
            const promises = Array.from(selectedSkus).map(sku =>
                axios.post(`${API_CONFIG.BASE_URL}/settings/priority-bottom`, { sku })
    ...
    const handleDelete = async () => {
        ...
        try {
            if (isBulkDeleting) {
                ...
                await axios.post(`${API_CONFIG.BASE_URL}/settings/priority-bottom/bulk-delete`, { ids });
                if (showToast) showToast(`✓ ${ids.length} SKU berhasil dihapus`);
            } else {
                await axios.delete(`${API_CONFIG.BASE_URL}/settings/priority-bottom/${encodeURIComponent(deleteCandidate)}`);
    ...
    const toggleSelectAllMain = () => {
        if (selectedSkusMain.size === priorityList.length) {
            setSelectedSkusMain(new Set());
        } else {
            setSelectedSkusMain(new Set(priorityList.map(item => item.sku)));
        }
    };
    ...
    const handleExportSelected = async () => {
        ...
        try {
            const ids = Array.from(selectedSkusMain);
            const response = await axios.post(`${API_CONFIG.BASE_URL}/settings/priority-bottom/export`, 
                { ids }, 
                { responseType: 'blob' }
            );
            ...
            link.href = url;
            link.setAttribute('download', `Export_SKU_Priority_${new Date().getTime()}.xlsx`);
    ...
    const handleExportAll = async () => {
        ...
        try {
            ...
            const link = document.createElement('a');
            link.href = url;
            link.setAttribute('download', `Export_ALL_SKU_Priority_${new Date().getTime()}.xlsx`);
            document.body.appendChild(link);
            link.click();
            link.remove();
        } catch (err) {
    ...
    const handleImport = async (file: File) => {
        ...
        try {
            const res = await axios.post(`${API_CONFIG.BASE_URL}/settings/import-priority-bottom`, formData, {
            ...
            <div className="flex justify-between items-center bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
                <div>
                    <h2 className="text-lg font-bold text-gray-900 flex items-center gap-2">
                        <FiList className="w-5 h-5" /> SKU Posisi Khusus (BOX/SLOP/PACK)
                    </h2>
                    <p className="text-sm text-gray-500 mt-1 max-w-2xl">
                        SKU yang ada di daftar ini akan diletakkan di urutan <strong>PALING BAWAH</strong> pada Packing List Excel, 
                        namun akan diletakkan di urutan <strong>PALING ATAS</strong> pada masing-masing resi (jika lebih dari 1 SKU) di Custom Label.
                    </p>
                </div>
                            ...
                            <tr>
                                <th className="px-6 py-4 font-semibold text-gray-700 w-12 text-center">
                                    <input
                                        ...
                                        checked={priorityList.length > 0 && selectedSkusMain.size === priorityList.length}
                                    ...
                                    >
                                        ...
                                        <td className="px-6 py-4 text-right" onClick={(e) => e.stopPropagation()}>
                                            <button
                                                ...
                                                title="Hapus dari Bottom Priority"
                                            >
...
export default AdminSkuPriority;
