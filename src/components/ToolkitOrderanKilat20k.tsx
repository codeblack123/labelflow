...
interface ToolkitOrderanKilat20kProps {
    showToast?: (message: string) => void;
}

const ToolkitOrderanKilat20k: React.FC<ToolkitOrderanKilat20kProps> = ({ showToast }) => {
    ...
    const handleProcess = async () => {
        ...
        try {
            console.log('[Toolkit] Mengirim ke orderan-kilat-20k...');

            const response = await axios.post(`${API_CONFIG.BASE_URL}/toolkit/orderan-kilat-20k`, formData, {
                responseType: 'blob',
            });
            ...
            setDownloadUrl(url);
            if (showToast) showToast('Data Orderan Kilat (>20K) berhasil diproses!');
    ...
    return (
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden animate-in fade-in slide-in-from-bottom-4">
            <div className="p-6 border-b border-gray-100 bg-gray-50/50 flex justify-between items-start">
                <div>
                    <h3 className="text-lg font-bold text-gray-900">Orderan Kilat (Filter SKU VIP &gt;20K)</h3>
                    <p className="text-sm text-gray-500 mt-1">Upload Excel Ginee, lalu sistem akan memfilter dan menyisakan data resi yang mengandung MSKU dari daftar SKU VIP (&gt;20K).</p>
                </div>
            </div>
            ...
            <div className="p-6 space-y-6">
                {/* Upload Zone */}
                <div 
                    className={`border-2 border-dashed rounded-xl p-8 text-center transition-all ${file ? 'border-purple-400 bg-purple-50/30' : 'border-gray-300 hover:border-purple-400 hover:bg-gray-50'}`}
                >
                    <input
                        type="file"
                        accept=".xlsx,.xls"
                        ...
                        className="hidden"
                        id="excel-upload-kilat-20k"
                    />
                    <label 
                        htmlFor="excel-upload-kilat-20k"
                    ...
                    <div className="bg-green-50 border border-green-200 text-green-700 p-4 rounded-lg flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-green-600 mt-0.5">File Excel Orderan Kilat (&gt;20K) sudah siap diunduh.</p>
                            </div>
                        </div>
                        <a 
                            href={downloadUrl}
                            download="Hasil_Orderan_Kilat_20K.xlsx"
                        ...
                        className={`w-full py-3.5 rounded-xl text-white font-bold text-sm flex items-center justify-center gap-2 transition-all ${
                            !file 
                                ...
                                : processing
                                    ...
                                    : 'bg-purple-600 hover:bg-purple-700 hover:shadow-lg hover:shadow-purple-200'
                        }`}
                    >
                        {processing ? (
                            <>
...
export default ToolkitOrderanKilat20k;
