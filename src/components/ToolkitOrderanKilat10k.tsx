...
interface ToolkitOrderanKilat10kProps {
    showToast?: (message: string) => void;
}

const ToolkitOrderanKilat10k: React.FC<ToolkitOrderanKilat10kProps> = ({ showToast }) => {
    ...
    const handleProcess = async () => {
        ...
        try {
            console.log('[Toolkit] Mengirim ke orderan-kilat-10k...');

            const response = await axios.post(`${API_CONFIG.BASE_URL}/toolkit/orderan-kilat-10k`, formData, {
                responseType: 'blob',
            });
            ...
            setDownloadUrl(url);
            if (showToast) showToast('Data Orderan Kilat (>10K) berhasil diproses!');
    ...
    return (
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden animate-in fade-in slide-in-from-bottom-4">
            <div className="p-6 border-b border-gray-100 bg-gray-50/50 flex justify-between items-start">
                <div>
                    <h3 className="text-lg font-bold text-gray-900">Orderan Kilat (Filter SKU VIP &gt;10K)</h3>
            ...
            <div className="p-6 space-y-6">
                {/* Upload Zone */}
                <div 
                    className={`border-2 border-dashed rounded-xl p-8 text-center transition-all ${file ? 'border-amber-400 bg-amber-50/30' : 'border-gray-300 hover:border-amber-400 hover:bg-gray-50'}`}
                >
                    <input
                        type="file"
                        accept=".xlsx,.xls"
                        ...
                        className="hidden"
                        id="excel-upload-kilat-10k"
                    />
                    <label 
                        htmlFor="excel-upload-kilat-10k"
                    ...
                    <div className="bg-green-50 border border-green-200 text-green-700 p-4 rounded-lg flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-green-600 mt-0.5">File Excel Orderan Kilat (&gt;10K) sudah siap diunduh.</p>
                            </div>
                        </div>
                        <a 
                            href={downloadUrl}
                            download="Hasil_Orderan_Kilat_10K.xlsx"
...
export default ToolkitOrderanKilat10k;
