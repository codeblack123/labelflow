...
interface AdminSkuVipProps {
    showToast?: (message: string) => void;
}

const AdminSkuVip: React.FC<AdminSkuVipProps> = ({ showToast }) => {
    ...
    const fetchSkuVipList = async () => {
        setIsLoading(true);
        try {
            const res = await axios.get(`${API_CONFIG.BASE_URL}/settings/sku-vip`);
    ...
    const handleSaveSelection = async () => {
        ...
        try {
            ...
            const promises = Array.from(selectedSkus).map(sku =>
                axios.post(`${API_CONFIG.BASE_URL}/settings/sku-vip`, { sku })
    ...
    const handleDelete = async () => {
        ...
        try {
            if (isBulkDeleting) {
                ...
                await axios.post(`${API_CONFIG.BASE_URL}/settings/sku-vip/bulk-delete`, { ids });
                if (showToast) showToast(`✓ ${ids.length} SKU berhasil dihapus`);
            } else {
                await axios.delete(`${API_CONFIG.BASE_URL}/settings/sku-vip/${encodeURIComponent(deleteCandidate)}`);
    ...
    const handleExportSelected = async () => {
        ...
        try {
            const ids = Array.from(selectedSkusMain);
            const response = await axios.post(`${API_CONFIG.BASE_URL}/settings/sku-vip/export`, 
                { ids }, 
                { responseType: 'blob' }
            );
    ...
    const handleImport = async (file: File) => {
        ...
        try {
            const res = await axios.post(`${API_CONFIG.BASE_URL}/settings/import-sku-vip`, formData);
            ...
            <div className="flex justify-between items-center bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
                <div>
                    <h2 className="text-lg font-bold text-gray-900 flex items-center gap-2">
                        <FiList className="w-5 h-5" /> Data SKU VIP (&gt;10K)
                    </h2>
                                    ...
                                    >
                                        ...
                                        <td className="px-6 py-4 text-right" onClick={(e) => e.stopPropagation()}>
                                            <button
                                                ...
                                                title="Hapus dari Bottom SKU VIP (&gt;10K)"
                                            >
