...
interface AdminSkuVip20kProps {
    showToast?: (message: string) => void;
}

const AdminSkuVip20k: React.FC<AdminSkuVip20kProps> = ({ showToast }) => {
    ...
    const fetchSkuVipList = async () => {
        setIsLoading(true);
        try {
            const res = await axios.get(`${API_CONFIG.BASE_URL}/settings/sku-vip-20k`);
    ...
    const handleSaveSelection = async () => {
        ...
        try {
            const promises = Array.from(selectedSkus).map(sku =>
                axios.post(`${API_CONFIG.BASE_URL}/settings/sku-vip-20k`, { sku })
                    .catch(err => console.warn(`Failed to add ${sku}`, err))
            );
            await Promise.all(promises);

            if (showToast) showToast(`✓ ${selectedSkus.size} SKU ditambahkan ke list SKU VIP (>20K)`);
            setIsModalOpen(false);
    ...
    const handleDelete = async () => {
        ...
        try {
            if (isBulkDeleting) {
                ...
                await axios.post(`${API_CONFIG.BASE_URL}/settings/sku-vip-20k/bulk-delete`, { ids });
                if (showToast) showToast(`✓ ${ids.length} SKU berhasil dihapus`);
            } else {
                await axios.delete(`${API_CONFIG.BASE_URL}/settings/sku-vip-20k/${encodeURIComponent(deleteCandidate)}`);
    ...
    const handleExportSelected = async () => {
        ...
        try {
            const ids = Array.from(selectedSkusMain);
            const response = await axios.post(`${API_CONFIG.BASE_URL}/settings/sku-vip-20k/export`, 
                { ids }, 
                { responseType: 'blob' }
            );
            ...
            link.href = url;
            link.setAttribute('download', `Export_SKU_SKU VIP (>20K)_${new Date().getTime()}.xlsx`);
    ...
    const handleExportAll = async () => {
        ...
        try {
            ...
            const link = document.createElement('a');
            link.href = url;
            link.setAttribute('download', `Export_ALL_SKU_SKU VIP (>20K)_${new Date().getTime()}.xlsx`);
            document.body.appendChild(link);
            link.click();
            link.remove();
        } catch (err) {
    ...
    const handleDownloadTemplate = () => {
        ...
        XLSX.utils.book_append_sheet(wb, ws, 'Template SKU VIP 20K');
        XLSX.writeFile(wb, 'Template_Import_SKU_VIP_20K.xlsx');
    };
    ...
    const handleImport = async (file: File) => {
        ...
        try {
            const res = await axios.post(`${API_CONFIG.BASE_URL}/settings/import-sku-vip-20k`, formData);
    ...
    return (
        <div className="h-[700px] animate-in fade-in duration-300 flex flex-col space-y-6">
            ...
            <div className="flex justify-between items-center bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
                <div>
                    <h2 className="text-lg font-bold text-gray-900 flex items-center gap-2">
                        <FiList className="w-5 h-5 text-purple-600" /> Data SKU VIP (&gt;20K)
                    </h2>
                    <p className="text-sm text-gray-500 mt-1 max-w-2xl">
                        Daftar SKU bernilai tinggi (&gt;20.000) untuk keperluan filter Orderan Kilat.
                    </p>
                </div>
                <div className="flex gap-2">
                    ...
                    <button
                        ...
                        className="px-5 py-2.5 bg-purple-600 text-white font-medium rounded-lg hover:bg-purple-700 transition-colors flex items-center gap-2 shadow-sm"
                    >
            ...
            {selectedSkusMain.size > 0 && (
                <div className="flex items-center gap-3 p-3 bg-purple-50 border border-purple-200 rounded-lg animate-in slide-in-from-top-2 duration-300">
                    <span className="text-sm font-medium text-purple-800">
                        {selectedSkusMain.size} item terpilih
                    </span>
                    <div className="h-4 w-px bg-purple-300 mx-1"></div>
                    <button
                        onClick={handleExportSelected}
                        disabled={isExporting}
                        className="text-sm font-semibold text-purple-700 hover:text-purple-900 flex items-center gap-1 px-2 py-1 rounded hover:bg-purple-100 transition-colors disabled:opacity-50"
                    >
            ...
            <DeleteConfirmationModal
                ...
                title={isBulkDeleting ? 'Hapus Terpilih' : 'Hapus SKU VIP (>20K)'}
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {isLoading ? (
                                ...
                                <tr>
                                    <td colSpan={4} className="px-6 py-12 text-center">
                                        <div className="flex flex-col items-center justify-center text-gray-400">
                                            ...
                                            <p>List SKU VIP (&gt;20K) masih kosong.</p>
                                        </div>
                                    </td>
                                </tr>
                            ) : (
                                paginatedList.map((item, idx) => (
                                    <tr 
                                        key={item.id} 
                                        className={`hover:bg-gray-50 transition-colors cursor-pointer ${selectedSkusMain.has(item.sku) ? 'bg-purple-50/50' : ''}`}
                    ...
                    <div className="flex items-center justify-between p-4 border-t border-gray-200 bg-gray-50 rounded-b-xl">
                        <div className="flex items-center gap-2">
                            ...
                            <select
                                ...
                                className="text-sm border border-gray-300 rounded-md focus:ring-purple-500 focus:border-purple-500"
                            >
                ...
                >
                    <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl flex flex-col max-h-[85vh] overflow-hidden" onClick={e => e.stopPropagation()}>
                        <div className="p-5 border-b border-gray-100 flex justify-between items-center bg-gray-50">
                            <div>
                                <h3 className="text-lg font-bold text-gray-900">Pilih SKU untuk VIP (&gt;20K)</h3>
                                <p className="text-xs text-gray-500">Pilih SKU dari database untuk dimasukkan ke list SKU VIP (&gt;20K).</p>
                            </div>
                        ...
                        <div className="p-4 border-b border-gray-100 space-y-3">
                            <div className="relative">
                                ...
                                <input
                                    ...
                                    className="w-full pl-10 pr-10 py-2 text-sm border border-gray-300 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-transparent outline-none"
                                    autoFocus
                                />
                                {searchTerm && (
                                    <button
                                    ...
                                    return (
                                        ...
                                        >
                                            <div className={`
                                                ...
                                                ${isSelected ? 'bg-purple-500 border-purple-500 text-white' : 'bg-white border-gray-300'}
                                            `}>
                        ...
                        <div className="p-5 border-t border-gray-100 bg-gray-50 flex justify-between items-center">
                            ...
                            <div className="flex gap-3">
                                ...
                                <button
                                    ...
                                    className="px-6 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 text-sm font-medium disabled:bg-purple-300 flex items-center gap-2"
                                >
