...
interface AdminSkuVip50kProps {
    showToast?: (message: string) => void;
}

const AdminSkuVip50k: React.FC<AdminSkuVip50kProps> = ({ showToast }) => {
    ...
    const fetchSkuVipList = async () => {
        setIsLoading(true);
        try {
            const res = await axios.get(`${API_CONFIG.BASE_URL}/settings/sku-vip-50k`);
    ...
    const handleSaveSelection = async () => {
        ...
        try {
            ...
            const promises = Array.from(selectedSkus).map(sku =>
                axios.post(`${API_CONFIG.BASE_URL}/settings/sku-vip-50k`, { sku })
    ...
    const handleDelete = async () => {
        ...
        try {
            if (isBulkDeleting) {
                ...
                await axios.post(`${API_CONFIG.BASE_URL}/settings/sku-vip-50k/bulk-delete`, { ids });
                if (showToast) showToast(`✓ ${ids.length} SKU berhasil dihapus`);
            } else {
                await axios.delete(`${API_CONFIG.BASE_URL}/settings/sku-vip-50k/${encodeURIComponent(deleteCandidate)}`);
    ...
    const handleExportSelected = async () => {
        ...
        try {
            const ids = Array.from(selectedSkusMain);
            const response = await axios.post(`${API_CONFIG.BASE_URL}/settings/sku-vip-50k/export`, 
                { ids }, 
                { responseType: 'blob' }
            );
            ...
            link.href = url;
            link.setAttribute('download', `Export_SKU_SKU VIP (>50K)_${new Date().getTime()}.xlsx`);
    ...
    const handleExportAll = async () => {
        ...
        try {
            ...
            const link = document.createElement('a');
            link.href = url;
            link.setAttribute('download', `Export_ALL_SKU_SKU VIP (>50K)_${new Date().getTime()}.xlsx`);
            document.body.appendChild(link);
            link.click();
            link.remove();
        } catch (err) {
    ...
    const handleDownloadTemplate = () => {
        ...
        XLSX.utils.book_append_sheet(wb, ws, 'Template SKU VIP');
        XLSX.writeFile(wb, 'Template_Import_SKU_VIP.xlsx');
    };
    ...
    const handleImport = async (file: File) => {
        ...
        try {
            const res = await axios.post(`${API_CONFIG.BASE_URL}/settings/import-sku-vip-50k`, formData);
            clearInterval(progressInterval);
            setUploadProgress(100);
            if (showToast) showToast(`V Import Sukses: ${res.data.count} SKU ditambahkan`);
            fetchSkuVipList();
        } catch (err: any) {
            clearInterval(progressInterval);
            ...
            <div className="flex justify-between items-center bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
                <div>
                    <h2 className="text-lg font-bold text-gray-900 flex items-center gap-2">
                        <FiList className="w-5 h-5" /> Data SKU VIP (&gt;50K)
                    </h2>
                                    ...
                                    >
                                        ...
                                        <td className="px-6 py-4 text-right" onClick={(e) => e.stopPropagation()}>
                                            <button
                                                ...
                                                title="Hapus dari Bottom SKU VIP (&gt;50K)"
                                            >
